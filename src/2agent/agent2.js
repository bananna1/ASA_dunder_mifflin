        ...
        case "intention":{
            switch (msg.intention_type){
                case "go_pick_up":{
                    ...
                    let my_int = myAgent.intention_queue[0].predicate;
                    if(reply){
...
class GoPickUp extends Plan {
    ...
    async execute(go_pick_up, x, y, id,) {
        ...
        for (let ag of friendly_agents){
            let other_is_better = await client.ask(ag,{type: "itention", intention_type:"go_pick_up",id, utility:my_utility});
            if( other_is_better){
